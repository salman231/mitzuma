 ...
 */
class Ozow extends \Magento\Payment\Model\Method\AbstractMethod
{
    /**
     * @var string
     */
     ...
     * @var string
     */
    protected $_formBlockType = 'Ozow\Ozow\Block\Form';

    /**
     * @var string
     */
    protected $_infoBlockType = 'Ozow\Ozow\Block\Payment\Info';
     ...
     *
     * @var \Ozow\Ozow\Model\Config $config
     */
    protected $_config;
     ...
     * @param \Ozow\Ozow\Model\ConfigFactory $configFactory
     * @param \Magento\Store\Model\StoreManagerInterface $storeManager
     * @param \Magento\Framework\UrlInterface $urlBuilder
     * @param \Ozow\Ozow\Model\CartFactory $cartFactory
    ...
    /**
     * This is where we compile data posted by the form to Ozow
     * @return array
     */
    public function getStandardCheckoutFormFields()
    {
        ...
        $site_code = $this->_config->getValue( 'site_code' );
        $private_key = $this->_config->getValue('private_key');
        $is_test = ($this->_config->getValue( 'is_test' ) ? 'true' : 'false');
        $country_code = $this->_config->getMerchantCountry();
        ...
        $data = array(
            ...
            'TransactionReference' => $order->getRealOrderId(),
            'BankReference' => $order->getRealOrderId(),
            'Optional5' => 'magento-v2.3',
            'Customer' => trim(sprintf('%s %s', $order->getData( 'customer_firstname' ), $order->getData( 'customer_lastname' ))),
            'CancelUrl' => $this->_urlBuilder->getUrl( 'ozow/redirect/cancel', array( '_secure' => true ) ) . '?form_key=' . $this->_formKey->getFormKey(),
            'SuccessUrl' => $this->_urlBuilder->getUrl( 'ozow/redirect/success', array( '_secure' => true ) ) . '?form_key=' . $this->_formKey->getFormKey(),
            'NotifyUrl' => $this->_urlBuilder->getUrl( 'ozow/notify', array( '_secure' => true ) ),
            'IsTest' => $is_test,
        );

        $hash_check = hash('sha512', strtolower(implode('', $data) . $private_key));

        $this->ozowLogger(strtolower(implode('', $data) . $private_key));

        $this->ozowLogger($hash_check);
    ...
    {
        return $this->_urlBuilder->getUrl( 'ozow/redirect/success', array( '_secure' => true ) );
    }
    ...
    {
        return $this->_urlBuilder->getUrl( 'ozow/redirect' );
    ...
    {
        return $this->_urlBuilder->getUrl( 'ozow/notify', array( '_secure' => true ) );
    }

    public function ozowLogger($message = '', $close = false )
    {
        $debug = ($this->_config->getValue( 'debug' ) == '1' ? true : false);
        ...
        if ($debug)
        {
            ...
            else
            {
                if (!$fh)
                {
                    ...
                    $fh = fopen($pathinfo['dirname'] . '/ozow.log', 'a+');
                }
