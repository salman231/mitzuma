...
/**
 * Ozow-specific model for shopping cart items and totals
 * The main idea is to accommodate all possible totals into Ozow-compatible 4 totals and line items
 */
class Cart extends \Magento\Payment\Model\Cart
{
    ...
    /**
     * Check the line items and totals according to Ozow business logic limitations
     *
     * @return void
     */
    protected function _validate()
    {
        ...
        if ( empty( $this->_transferFlags[self::AMOUNT_DISCOUNT] ) ) {
            ...
            // Ozow requires to have discount less than items subtotal
    ...
    /**
     * Import items from sales model with workarounds for Ozow
     *
     * @return void
     */
    protected function _importItemsFromSalesModel()
    {
        ...
        foreach ( $this->_salesModel->getAllItems() as $item ) {
            ...
            // Workaround in case if item subtotal precision is not compatible with Ozow (.2)
